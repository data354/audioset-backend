...
let storage = new Storage({
  projectId: "audiosetrecorder-2022",
  apiEndpoint: env.STORAGE_API_ENDPOINT,
  keyFilename: join(cwd(), ".cqx/keys/gs.json"),
});
...
router
      ...
      blobStream.on("finish", async () => {
        ...
        sound
          ...
          .catch((error: Error) => {
            ...
            res
              .status(500)
              .json({ error: "InternalError", message: "Something wrong" });
          });
      });
  ...
  .get(
    ...
    async (req: express.Request, res: express.Response) => {
      sound
        .getNewAudio(Number(req.params.userId))
        .then(data => {
          res.status(200).json(data);
        })
        .catch((error: Error) => {
  ...
  .get(
    "/count/recorded/:userId",
    async (req: express.Request, res: express.Response) => {
      sound
        .count({ recorded: true, UserId_: Number(req.params.userId) })
        .then(data => {
          res.status(200).json(data);
        })
        .catch((error: Error) => {
  ...
  .get(
    "/count/unrecorded",
    async (req: express.Request, res: express.Response) => {
      sound.count({ recorded: false }).then(data => {
        res.status(200).json(data);
      });
      // .catch((error: Error) => {
      //     console.error(error);
  ...
  .get("/recorded", async (req: express.Request, res: express.Response) => {
    sound
      .getAll({ where: { recorded: true }, include: { User_: true } })
      .then(data => {
        res.json(data);
      });
    // .catch((error: Error) => {
