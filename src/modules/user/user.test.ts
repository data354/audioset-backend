...
describe("user routes tester", () => {
  ...
  describe("/GET User", () => {
    it("Try to get all user", async () => {
      await request.get(`/user`).expect("Content-Type", /json/).expect(200);
    });
    ...
    it("/GET/id Try to get user xho not exist", async () => {
      await request.get(`/user/0`).expect("Content-Type", /json/).expect(404);
    });

    it("/GET/id Try with unexpected id", async () => {
      await request.get(`/user/un`).expect("Content-Type", /json/).expect(500);
    });
  });
});
